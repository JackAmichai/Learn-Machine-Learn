...
export class NeuralNetwork {
  ...
  /**
   * Gets connection weights asynchronously as a Float32Array.
   * Prevents UI blocking during large model updates.
   * @param {number} layerIndex - Index of the connection layer
  ...
  async getConnectionWeightsAsync(layerIndex) {
    ...
    try {
      ...
      if (!weights.length) return null;
      const kernel = weights[0];
      // data() returns a promise that resolves to the values
