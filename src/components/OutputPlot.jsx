...
export function OutputPlot({ model, data, modelVersion }) {
    ...
    useEffect(() => {
        ...
        async function drawPlot() {
            let inputTensor, preds;
            try {
                inputTensor = tf.tensor2d(inputs);
                preds = model.predict(inputTensor);
                // Optimization: Replaced synchronous preds.dataSync() with async preds.data()
                // This prevents blocking the main UI thread during tensor data extraction,
                // avoiding UI jank and improving overall render performance.
                const predData = await preds.data();

                if (!isMounted) return;

                // Explicitly clear canvas before drawing to avoid opacity buildup
                ...
                for (let i = 0; i < gridSize; i++) {
                    for (let j = 0; j < gridSize; j++) {
                        const val = predData[i * gridSize + j];
                ...
                console.error("Error drawing OutputPlot:", err);
            } finally {
