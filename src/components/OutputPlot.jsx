...
export function OutputPlot({ model, data, modelVersion }) {
    ...
    useEffect(() => {
        ...
        async function drawPlot() {
            let inputTensor, predictionsTensor;
            try {
                inputTensor = tf.tensor2d(inputs);
                predictionsTensor = model.predict(inputTensor);

                // ⚡ Bolt Optimization: Use asynchronous await predictionsTensor.data()
                // instead of synchronous .dataSync() to prevent blocking the main JS thread.
                // This ensures the UI remains responsive during continuous training updates.
                const preds = await predictionsTensor.data();

                if (!isMounted) return;

                // Clear the canvas after the await to prevent any visual flickering
                // during the async data loading process.
