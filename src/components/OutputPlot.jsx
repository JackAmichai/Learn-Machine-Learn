    ...
    useEffect(() => {
        ...
        const drawPlot = async () => {
            ...
            try {
                ...
                // Replaced synchronous `dataSync()` with asynchronous `await tensor.data()`.
                // `dataSync()` blocks the main thread waiting for the GPU to finish computation,
                // causing severe UI jank. Awaiting `data()` allows the UI to remain responsive.
                const preds = await predsTensor.data();

                if (!isMounted) return;

                // Explicitly clear the canvas to prevent opacity buildup
