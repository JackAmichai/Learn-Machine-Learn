...
export function OutputPlot({ model, data, modelVersion }) {
    ...
    useEffect(() => {
        ...
        // ⚡ Bolt: Replace blocking dataSync() with async await data() to prevent main thread blocking
        // We use an async IIFE to handle the promise inside useEffect.
        // We wrap the tensor creation and prediction inside tf.tidy(), returning the final prediction tensor,
        // await its .data() outside the tidy block, and manually dispose of only the final tensor in a try/finally block.
        (async () => {
            let predsTensor;
            try {
                predsTensor = tf.tidy(() => {
                ...
                ctx.clearRect(0, 0, width, height); // ensure synchronous canvas clearing operations like ctx.clearRect() occur *after* the await call
