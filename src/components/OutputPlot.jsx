...
export function OutputPlot({ model, data, modelVersion }) {
    ...
    useEffect(() => {
        ...
        async function renderPlot() {
            // ⚡ Bolt: Use async data() to prevent UI freezing during large tensor reads
            let finalPredsTensor;
            try {
                finalPredsTensor = tf.tidy(() => {
                    const inputTensor = tf.tensor2d(inputs);
                    return model.predict(inputTensor);
                });

                const preds = await finalPredsTensor.data();

                if (!isActive) return;

                // Clear before drawing async to prevent race condition erasure
