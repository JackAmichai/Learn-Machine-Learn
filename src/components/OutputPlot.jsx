    ...
    useEffect(() => {
        ...
        async function renderPlot() {
            ...
            try {
                ...
                // Bolt optimization: use await predsTensor.data() instead of dataSync()
                // to avoid blocking the main thread during render.
                const preds = await predsTensor.data();

                if (!isMounted) return;

                // Clear canvas to prevent opacity buildup across frames
                ...
                // 2. Draw Data Points (moved here to ensure drawn on top of heatmap after async operation)
                if (data.points) {
    ...
    return (
        <div style={{ position: 'relative', width: '100%', height: '100%' }}>
            <canvas
                ...
                height={400}
                role="img"
                aria-label="Model decision boundary and data points"
                style={{
                    width: '100%',
                    height: '100%',
                    borderRadius: '12px',
