...
export function OutputPlot({ model, data, modelVersion }) {
    ...
    useEffect(() => {
        ...
        const renderPlot = async () => {
            // Keep tensors inside tidy, but explicitly return the final prediction tensor to avoid premature disposal
            ...
            try {
                // Async data retrieval to avoid blocking UI thread
                const preds = await predsTensor.data();

                if (!isActive) return;

                // Clear before drawing since it's async now
                ...
                // Manually dispose of the tensor since we returned it from tidy
                if (predsTensor) {
                    predsTensor.dispose();
                }
            }
