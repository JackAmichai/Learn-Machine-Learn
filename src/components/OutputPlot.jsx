...
export function OutputPlot({ model, data, modelVersion }) {
    ...
    useEffect(() => {
        ...
        async function draw() {
            let predsTensor;
            try {
                // ⚡ Bolt: Wrap prediction in tf.tidy but return tensor to await .data() without blocking UI thread
                predsTensor = tf.tidy(() => {
                ...
                // ⚡ Bolt: Clear canvas synchronously after await to prevent flickering
