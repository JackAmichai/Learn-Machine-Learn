...
export function OutputPlot({ model, data, modelVersion }) {
    ...
    useEffect(() => {
        ...
        async function drawOutput() {
            let predTensor;
            try {
                // ⚡ Bolt: Wrap tensor creation and prediction in tf.tidy, returning final tensor
                predTensor = tf.tidy(() => {
                ...
                // ⚡ Bolt: Prevent race conditions by checking stale flag before rendering
                if (isStale) return;
                ...
                console.error("OutputPlot rendering error:", error);
            } finally {
                // ⚡ Bolt: Manually dispose the final tensor to prevent memory leaks
                if (predTensor) {
                    predTensor.dispose();
                }
            }
        ...
        drawOutput().catch(console.error);
