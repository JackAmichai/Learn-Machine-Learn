...
// ⚡ Bolt: Extract static grid to avoid recreating 2500-element array on every render
...
export function OutputPlot({ model, data, modelVersion }) {
    ...
    useEffect(() => {
        let isStale = false; // ⚡ Bolt: cancellation token
        ...
        const drawPlotAsync = async () => {
            let inputTensor, predsTensor;
            try {
                // ⚡ Bolt: Convert synchronous operations to asynchronous to avoid blocking main thread
                inputTensor = tf.tensor2d(staticInputs);
                ...
                // ⚡ Bolt: explicitly manage memory without tf.tidy
