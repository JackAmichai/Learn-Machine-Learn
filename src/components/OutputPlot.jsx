...
export function OutputPlot({ model, data, modelVersion }) {
    ...
    useEffect(() => {
        ...
        async function drawPlot() {
            ...
            // Use async extraction instead of blocking dataSync
            const preds = await predTensor.data();

            // Manually dispose of tensors since we can't use tf.tidy with async
            inputTensor.dispose();
            predTensor.dispose();

            if (!isMounted) return;

            // Clear canvas after await to prevent opacity buildup
