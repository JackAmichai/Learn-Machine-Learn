...
export function OutputPlot({ model, data, modelVersion }) {
    ...
    useEffect(() => {
        ...
        const renderPlot = async () => {
            ...
            // We must manually dispose predsTensor because we are awaiting its data outside tidy
            let predsTensor;
            try {
                predsTensor = tf.tidy(() => {
                    const inputTensor = tf.tensor2d(inputs);
                    // Return the tensor itself, NOT dataSync()
                    return model.predict(inputTensor);
                });

                // Fetch data async from GPU, preventing main thread block
