...
export function OutputPlot({ model, data, modelVersion }) {
    ...
    useEffect(() => {
        ...
        const drawPlot = async () => {
            try {
                // ⚡ Bolt: Optimize performance by moving prediction out of blocking tf.tidy
                // This prevents main thread jank caused by dataSync() during model visualization updates
                predsTensor = tf.tidy(() => {
                    const inputTensor = tf.tensor2d(inputs);
                    return model.predict(inputTensor);
                });

                // ⚡ Bolt: Use async tensor.data() to unblock main thread
