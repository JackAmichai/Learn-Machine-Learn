...
export function OutputPlot({ model, data, modelVersion }) {
    ...
    useEffect(() => {
        ...
        const draw = async () => {
            let predsTensor;
            try {
                // ⚡ Bolt: Use async data() to prevent blocking the UI thread during drawing.
                // Wrap in tf.tidy to clean up intermediate tensors, but return final tensor
                predsTensor = tf.tidy(() => {
                ...
                // Clear context ONLY after async call to prevent flickering
