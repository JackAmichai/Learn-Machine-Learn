...
export function OutputPlot({ model, data, modelVersion }) {
    ...
    useEffect(() => {
        ...
        // Optimization: Use async data extraction to avoid blocking the main UI thread.
        // Synchronous dataSync() causes severe jank during frequent React re-renders.
        async function drawPlot() {
            let predsTensor;
            try {
                // Wrap only the tensor creation and prediction inside tf.tidy.
                predsTensor = tf.tidy(() => {
                ...
                // Impact: Smooths UI animations and prevents playground freezing.
                const preds = await predsTensor.data();

                // Prevent race conditions if the component unmounts or re-renders rapidly.
                if (!isActive) return;
