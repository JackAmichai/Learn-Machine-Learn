    ...
    useEffect(() => {
        ...
        async function renderPlot() {
            try {
                ...
                // ⚡ Bolt: Replace blocking dataSync() with async data() to prevent UI jank during render
                const preds = await predsTensor.data();

                if (!isMounted) return;

                // Clear canvas before drawing to prevent opacity buildup from previous async frames
                ...
                console.error("Error rendering OutputPlot:", err);
            } finally {
