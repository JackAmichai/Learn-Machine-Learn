...
export function OutputPlot({ model, data, modelVersion }) {
    ...
    useEffect(() => {
        ...
        async function drawPlot() {
            // tf.tidy does not support async callbacks directly if returning promises
            // So we manually dispose the resulting tensor
            ...
            try {
                // Use async extraction to avoid blocking main thread
                const preds = await predsTensor.data();

                if (!isActive) return;

                // Clear previous drawings *after* await to avoid flickering
