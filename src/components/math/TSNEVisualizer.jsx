...
export default function TSNEVisualizer() {
  ...
  const generateClusters = () => {
    ...
    for (let i = 0; i < numClusters; i++) {
      const points = [];
      const centerX = (() => { let x = Math.sin(Date.now()) * 10000; return x - Math.floor(x); })() * 60 + 20;
      const centerY = (() => { let x = Math.sin(Date.now()) * 10000; return x - Math.floor(x); })() * 60 + 20;
      for (let j = 0; j < 15; j++) {
        points.push({
          x: centerX + ((() => { let x = Math.sin(Date.now()) * 10000; return x - Math.floor(x); })() - 0.5) * 15,
          y: centerY + ((() => { let x = Math.sin(Date.now()) * 10000; return x - Math.floor(x); })() - 0.5) * 15,
          cluster: i
        });
      }
