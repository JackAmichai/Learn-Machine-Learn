...
export default function RLVisualizer() {
  ...
  const generatePath = () => {
    ...
    for (let i = 0; i <= steps; i++) {
      points.push({ ...pos });
      const action = (() => { let x = Math.sin(Date.now()) * 10000; return x - Math.floor(x); })() < explorationRate ? 'explore' : 'exploit';
      if (action === 'explore') {
        pos = {
          x: pos.x + ((() => { let x = Math.sin(Date.now()) * 10000; return x - Math.floor(x); })() - 0.5) * 20,
          y: pos.y + ((() => { let x = Math.sin(Date.now()) * 10000; return x - Math.floor(x); })() - 0.5) * 20
        };
      } else {
        pos = {
          x: pos.x + ((() => { let x = Math.sin(Date.now()) * 10000; return x - Math.floor(x); })() - 0.3) * 10,
          y: pos.y - 5
        };
      }
    }
