...
export default function RLVisualizer() {
  ...
  const generatePath = () => {
    const points = [];
    let pos = { x: 0, y: 0 };
    let seed = episode * 1000 + steps; // Stable seed based on props
    const nextRandom = () => {
    ...
    for (let i = 0; i <= steps; i++) {
      points.push({ ...pos });
      const action = nextRandom() < explorationRate ? 'explore' : 'exploit';
      if (action === 'explore') {
        pos = {
          x: pos.x + (nextRandom() - 0.5) * 20,
          y: pos.y + (nextRandom() - 0.5) * 20
        };
      } else {
        pos = {
          x: pos.x + (nextRandom() - 0.3) * 10,
          y: pos.y - 5
        };
      }
    }
