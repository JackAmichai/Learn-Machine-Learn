...
async function computeMetrics(model, data, modelVersion) {
    ...
    try {
        let preds;
        tf.tidy(() => {
            // Predict inside tidy to clean up intermediate tensors inside predict
            preds = model.predict(data.xs);
            // We must keep the final prediction tensor so we can await its data outside tidy
            tf.keep(preds);
        });

        // For binary classification with single output node
        const predData = await preds.data();
        preds.dispose(); // clean up the prediction tensor manually
