...
## 2024-10-24 - Synchronous TensorFlow.js Operations in Render Loop
**Learning:** TensorFlow.js `dataSync()` is a blocking synchronous operation that transfers data from GPU to CPU. Calling this inside a React component's render body (via an IIFE or `useEffect`) causes significant performance degradation as it blocks the main thread.
**Action:** Always wrap `dataSync()` or any heavy TF.js extraction logic with asynchronous `await data()` calls. When converting to async, ensure `tf.tidy()` is removed (as it does not support Promises) and explicitly manage memory by calling `.dispose()` on tensors in a `try/finally` block.
