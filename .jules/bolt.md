...
## 2024-07-06 - Async Tensor Predictions in React
**Learning:** Using `tf.tidy()` in a React `useEffect` with `dataSync()` blocks the main thread during heavy computations (like rendering a 50x50 prediction grid). However, converting to async `await tensor.data()` requires manually removing `tf.tidy()` and handling tensor disposal in a `try...finally` block.
**Action:** When making TensorFlow predictions asynchronous in React components, always manually track and dispose tensors in a `try...finally` block, and include an `isMounted` check to prevent updating canvas/state on unmounted components.
