...
## 2026-06-21 - TensorFlow.js dataSync in OutputPlot
**Learning:** Using `dataSync()` inside a React `useEffect` hook without async logic blocks the main thread during component renders. This is especially problematic in visualization components like `OutputPlot` which may update frequently.
**Action:** Use `await tensor.data()` instead, wrapping it in an async function within the `useEffect`. Manage memory manually with `dispose()` since `tf.tidy` does not support async.
