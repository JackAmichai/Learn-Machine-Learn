...
## 2024-11-20 - Asynchronous Tensor Extraction Lifecycle
**Learning:** `tf.tidy()` strictly requires synchronous functions. When refactoring components to use `await tensor.data()` instead of the blocking `dataSync()`, you must remove `tf.tidy()` and manually manage the lifecycle of tensors (e.g. `tensor.dispose()`) inside a `finally` block to prevent GPU memory leaks.
**Action:** Use manual `.dispose()` in async tensor extraction flows in React `useEffect` or `useMemo` hooks, and handle component unmounting with an `isMounted` flag.
