...
## 2024-07-19 - TF.js Synchronous Data Blocking in React Render
**Learning:** `dataSync()` blocks the main thread. Converting it to `.data()` in `useEffect` makes the UI more responsive, but requires replacing `tf.tidy()` with manual `.dispose()` in a `try/finally` block since `tf.tidy()` doesn't support promises, and also requires an `isStale` flag in the cleanup function to prevent out-of-order execution during rapid updates.
**Action:** When working with TF.js predictions in UI components, always use `.data()` instead of `.dataSync()` for responsiveness, manually clean up tensors in a `try/finally` block, and use a cancellation flag like `isStale` inside `useEffect`.
