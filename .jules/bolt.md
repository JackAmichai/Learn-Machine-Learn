...
## 2024-05-22 - OutputPlot Async Optimization
**Learning:** `dataSync()` blocks UI in `OutputPlot.jsx` during model predictions.
**Action:** Replace `preds.dataSync()` with asynchronous `await preds.data()` inside a `useEffect`. Handled disposal properly since `tf.tidy()` does not support async.
