...
## 2024-11-20 - Asynchronous Tensor Extraction in React UseEffect\n**Learning:** Using `tf.tidy()` with synchronous `dataSync()` blocks the main UI thread during renders. However, `tf.tidy()` cannot handle asynchronous operations like `await tensor.data()`.\n**Action:** When extracting tensor data asynchronously inside a React `useEffect`, manually manage memory by keeping references to intermediate tensors (`inputTensor`, `predsTensor`) and explicitly calling `.dispose()` on them within a `try...finally` block. Additionally, implement an `isMounted` flag to guard against state updates or canvas operations if the component unmounts while awaiting the GPU.
