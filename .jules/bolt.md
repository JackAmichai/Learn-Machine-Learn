...
## 2024-05-22 - TF.js Memory Management in Async Workflows
**Learning:** When replacing synchronous `tf.tidy()` and `tensor.dataSync()` calls with asynchronous `await tensor.data()` inside a loop (e.g., scanning layers for dead neurons), `tf.tidy()` cannot be used. Intermediate tensors created during iterations (like `layer.apply(current)`) will cause significant memory leaks if not manually tracked.
**Action:** Always maintain an array (e.g., `let tensorsToDispose = [current]`) when stepping through layers asynchronously, push intermediate and transient tensors (like condition masks) to it, and dispose of them all in a `finally` block to ensure memory correctness.
