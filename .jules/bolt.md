...
## 2025-02-19 - [Async Tensor Data Fetching]
**Learning:** `tf.tidy` cannot manage tensors that are awaited inside it because it synchronously disposes everything. The pattern is to create tensors in `tf.tidy`, return them (to keep them alive), await their data, and then manually dispose them.
**Action:** When converting sync `dataSync()` to async `data()`, ensure `tf.tidy` returns the tensors, and use a `try/finally` block to dispose them after `await Promise.all(...)`.
