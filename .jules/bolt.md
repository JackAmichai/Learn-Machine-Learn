...
## 2025-02-28 - TF.js rendering loop sync canvas clearing
**Learning:** When migrating synchronous canvas rendering to an asynchronous loop (e.g., replacing `dataSync()` with `await data()`), ensure all sequential drawing operations (e.g., rendering superimposed data points) are moved inside the asynchronous block after `clearRect()`. Leaving some rendering logic outside the async block creates a race condition where the async clear operation erases the synchronously drawn elements.
**Action:** Move all dependent and sequential rendering logic inside the asynchronous execution path after the await is resolved and the canvas is cleared.
