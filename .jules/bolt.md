...
## 2024-03-07 - TF.js dataSync() inside tf.tidy()
**Learning:** When replacing synchronous `dataSync()` with asynchronous `await tensor.data()`, you cannot use `tf.tidy()` because it only handles synchronous code. Instead, you must manually dispose of tensors in a `finally` block to prevent GPU memory leaks.
**Action:** When extracting tensor data asynchronously using `await tensor.data()`, instantiate the tensors outside a `try` block and explicitly call `.dispose()` on them within a `finally` block.
