...
## 2024-05-22 - OutputPlot asynchronous tensor extraction
**Learning:** `dataSync()` blocks the main thread in React during active training loops.
**Action:** Replaced `predsTensor.dataSync()` in `src/components/OutputPlot.jsx` with an asynchronous `await predsTensor.data()`. Required adding manual tensor disposal in a `try...finally` block (since `tf.tidy()` is strictly synchronous) and adding `isMounted` checks to safely handle asynchronous canvas drawing without memory leaks.
